...
class CMBModel(object):
    ...
    def init_params(self, options):
        ...
        # decoder: CMB
        params = self.cmb_layers.get_layer('cmb_cond')[0](options, params, prefix='bo_cmb',
                                                          nin=options['dim_word'], dim=options['dim'],
                                                          dimctx=ctx_dim)
        params = self.cmb_layers.get_layer('cmb')[0](params, nin=options['dim'], dim=options['dim'],
                                                     prefix='to_cmb')
    ...
    def build_model(self, tparams, options):
        ...
        # decoder
        bo_lstm = self.cmb_layers.get_layer('cmb_cond')[1](tparams, emb, options,
                                                           prefix='bo_cmb',
                                                           ...
                                                           trng=trng,
                                                           use_noise=use_noise)
        to_lstm = self.cmb_layers.get_layer('cmb')[1](tparams, bo_lstm[0],
                                                      mask=mask,
                                                      one_step=False,
                                                      prefix='to_cmb'
                                                      )
