...
from lstm_nonlocal_layers import LSTMNonLocalLayers


class LSTMNonLocalModel(object):
    def __init__(self):
        self.layers = Layers()
        # self.non_local_layers = NonLocalLayers()
        self.lstm_non_local_layers = LSTMNonLocalLayers(self.layers)
    ...
    def init_params(self, options):
        ...
        #                                                                c=options['ctx_dim'])
        params = self.lstm_non_local_layers.get_layer('lstm_non_local_layer')[0](options, params,
                                                                                 prefix='lstm_non_local_layer',
                                                                                 twh=options['K'],
                                                                                 c=options['ctx_dim'])
        return params
    ...
    def build_model(self, tparams, options):
        ...
        # non local layer
        self_att_ctx = self.lstm_non_local_layers.get_layer('lstm_non_local_layer')[1](tparams, ctx, options,
                                                                                       prefix='lstm_non_local_layer')
    ...
    def build_sampler(self, tparams, options, use_noise, trng, mode=None):
        ...
        # non local layer
        ctx0 = self.lstm_non_local_layers.get_layer('lstm_non_local_layer')[1](tparams, ctx0, options,
                                                                               prefix='lstm_non_local_layer')
