...
def score_with_cocoeval(samples_valid, samples_test, engine):
    ...
    if samples_valid:
        ...
        pkl.dump(gts_valid, open('gts_valid.pkl', 'w'))
        pkl.dump(samples_valid, open('samples_valid.pkl', 'w'))
    ...
    if samples_test:
        ...
        pkl.dump(gts_test, open('gts_test.pkl', 'w'))
        pkl.dump(samples_test, open('samples_test.pkl', 'w'))
        test_score = scorer.score(gts_test, samples_test, engine.test_ids)
    else:
        test_score = None
    return valid_score, test_score, scorer.imgToEval
...
def compute_score(
    ...
    valid_score, test_score, imgToEval = score_with_cocoeval(samples_valid, samples_test, engine)
    scores_final = {}
    scores_final['valid'] = valid_score
    scores_final['test'] = test_score
    scores_final['imgToEval'] = imgToEval
    if one_time:
        return scores_final
